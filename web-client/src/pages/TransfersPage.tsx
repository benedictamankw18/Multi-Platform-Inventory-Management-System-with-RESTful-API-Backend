                  ...
                  {isRejected ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 8, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)' }}>
                      ...
                      <div>
                        ...
                        {t.approved_by_name && <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>by {t.approved_name || t.approved_by_name} {t.approved_at ? `· ${fmtDate(t.approved_at)} ${fmtTime(t.approved_at)}` : ''}</div>}
                      </div>
                    </div>
                  ) : (
