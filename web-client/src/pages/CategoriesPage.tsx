...
export default function CategoriesPage() {
  ...
  async function getExportData(): Promise<Record<string, unknown>[]> {
    try {
      ...
      const parentMap = Object.fromEntries(allCategories.map((c: Category) => [c.category_id, c.category_name]))
      return allCategories.map((c: Category) => ({
            ...
            <tbody>
              {loading ? (
                ...
                return (<tr key={c.category_id} style={{ opacity: c.is_active ? 1 : 0.5 }}>
                  <td>
                    {c.image_url ? (
                      <img src={resolveImageUrl(c.image_url) ?? undefined} alt="" style={{ width: 40, height: 40, borderRadius: 6, objectFit: 'cover' }} />
                    ) : (
