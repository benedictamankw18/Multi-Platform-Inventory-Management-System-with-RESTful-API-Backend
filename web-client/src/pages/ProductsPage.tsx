...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    getBranches().then((d) => setBranches(Array.isArray(d) ? d : d?.data ?? d?.branches ?? [])).catch(() => {})
  }, [])
  ...
  function openEdit(p: Product) {
    ...
    getInventory({ productId: p.product_id }).then((res) => {
      const data = Array.isArray(res) ? res : res?.data ?? res?.inventory ?? []
      if (data.length > 0) {
        const inv = data[0]
    ...
    <div className="page">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={p.product_id} style={{ opacity: p.is_active ? 1 : 0.5 }}>
                  <td>
                    {p.primary_image_url ? (
                      <img src={resolveImageUrl(p.primary_image_url)} alt="" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 'var(--radius-button)', border: '1px solid var(--border)' }} />
                    ) : (
                ...
                <div>
                  {viewImages.length > 0 ? (
                    <>
                      ...
                      >
                        <img src={resolveImageUrl(viewImages[0].image_url)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      </div>
                      {viewImages.length > 1 && (
      ...
      {lightboxIndex !== null && viewImages.length > 0 && (
        ...
        >
          ...
          {/* Image */}
          <img
            src={resolveImageUrl(viewImages[lightboxIndex].image_url)}
            alt=""
