...
exports.createInventoryRecord = async ({ inventory_id, product_id, branch_id, quantity_on_hand = 0, reorder_level = 0, reorder_quantity = 0, reserved_quantity = 0, damaged_quantity = 0, expired_quantity = 0, available_quantity = 0, last_stock_take = null }, client = db) => {
...
exports.getInventoryById = async (inventory_id, client = db) => {
  const q = `SELECT * FROM ${TABLE} WHERE inventory_id = $1 LIMIT 1`;
  const { rows } = await client.query(q, [inventory_id]);
  return rows[0];
};

exports.getInventoryByProductAndBranch = async (product_id, branch_id, client = db) => {
  const q = `SELECT * FROM ${TABLE} WHERE product_id = $1 AND branch_id = $2 LIMIT 1`;
  const { rows } = await client.query(q, [product_id, branch_id]);
  return rows[0];
};

exports.listInventoriesByBranch = async (branch_id, { limit = 50, offset = 0 } = {}, client = db) => {
  const q = `SELECT * FROM ${TABLE} WHERE branch_id = $1 ORDER BY product_id LIMIT $2 OFFSET $3`;
  const { rows } = await client.query(q, [branch_id, limit, offset]);
  return rows;
};

exports.updateInventory = async (inventory_id, patch, client = db) => {
  const fields = [];
  const params = [];
  let idx = 1;
  ...
  if (!fields.length) return exports.getInventoryById(inventory_id, client);
