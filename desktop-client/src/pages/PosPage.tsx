                ...
                <div className='dropdown-results' style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--shadow-lg)', zIndex: 50, maxHeight: 320, overflowY: 'auto' }}>
                  {searchResults.map((p) => {
                    ...
                    return (
                      ...
                      >
                        {p.primary_image_url ? (
                          <img src={resolveImageUrl(p.primary_image_url) ?? undefined} alt="" style={{ width: 36, height: 36, objectFit: 'cover', borderRadius: 6, border: '1px solid var(--border)' }} />
                        ) : (
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                            {item.product.primary_image_url ? (
                              <img src={resolveImageUrl(item.product.primary_image_url) ?? undefined} alt="" style={{ width: 36, height: 36, objectFit: 'cover', borderRadius: 6, border: '1px solid var(--border)' }} />
                            ) : null}
                            <div>
          ...
          <div className="receipt-section receipt-totals">
            ...
            {Number(sale?.refunded_amount ?? 0) > 0 && (
              <div className="receipt-row" style={{ color: '#000000' }}>
                <span>Refunded</span>
                <span>{fmt(Number(sale?.refunded_amount ?? 0))}</span>
              </div>
            )}
          </div>
          ...
          <div className="receipt-section receipt-payment">
            ...
            {Number(sale?.balance_due ?? 0) > 0 && (
              <div className="receipt-row r-remain">
                ...
                <span>{fmt(Number(sale?.balance_due ?? 0))}</span>
              </div>
            )}
            {change > 0 && (
              <div className="receipt-row">
